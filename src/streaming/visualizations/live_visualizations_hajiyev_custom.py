...
def init_live_chart() -> tuple[Any, Any, list[int], list[float]]:
    ...
    axis.set_title("Premium Loyalty Points Tracker")
    axis.set_xlabel("Message")
    axis.set_ylabel("Loyalty Reward Points")
...
def update_live_chart(
    *,
    ...
    loyalty_points = float(message["total"]) * 10
    y_values.append(loyalty_points)
    # Clear the axis
    axis.clear()
    ...
    axis.scatter(
        x_values, y_values, color="purple", marker="x", s=100
